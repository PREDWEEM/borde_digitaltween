...
def load_seasonal_reference(
    ...
    curvas sean de Bordenave. Cada curva se normaliza por su propio total.
    """
...
def load_local_seasonal_reference(root: str | Path, as_of=None) -> pd.DataFrame:
    """Pool de ocho curvas históricas y Bordenave 2026, según disponibilidad.
    ...
    counts_path = root / "data/calibration/bordenave_2026_counts.csv"
    counts = pd.read_csv(counts_path)
    if not {"Fecha", "media(SR).m2"}.issubset(counts.columns) or len(counts) < 2:
        raise ValueError("La referencia 2026 requiere Fecha, media(SR).m2 y al menos dos visitas.")
    dates = pd.to_datetime(counts["Fecha"], errors="raise").dt.normalize()
    flows = pd.to_numeric(counts["media(SR).m2"], errors="raise").to_numpy(float)
    ...
    reference.attrs["source_2026"] = {
        "path": "data/calibration/bordenave_2026_counts.csv",
