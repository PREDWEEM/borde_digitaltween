...
def build_calibration(observations_path, weather_path, output_path, site="Bordenave",
                      coverage=50.0, w_max=18.8, source_metadata_path=None):
    ...
    last_count = pd.to_datetime(raw["Fecha"], errors="raise").max()
    ...
    validation = {
        ...
        "note": (
            ...
            "Se usa meteorología observada/provisional, no pronósticos archivados. No demuestra "
...
def main():
    ...
    parser.add_argument("--observations", type=Path, default=ROOT / "data/calibration/bordenave_2026_counts.csv")
    parser.add_argument("--weather", type=Path, default=ROOT / "data/calibration/bordenave_2026_weather.csv")
    parser.add_argument("--output", type=Path, default=ROOT / "data/calibration/bordenave_2026.json")
    parser.add_argument("--source-metadata", type=Path, default=ROOT / "data/calibration/bordenave_2026_source.json")
    parser.add_argument("--site", default="Bordenave")
    parser.add_argument("--coverage", type=float, default=50.0)
    parser.add_argument("--w-max", type=float, default=18.8)
