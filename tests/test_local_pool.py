"""Composición, peso por campaña y disponibilidad del pool de Bordenave."""
...
COUNTS = Path("data/calibration/bordenave_2026_counts.csv")
...
def test_nine_campaign_percentiles_have_equal_individual_weight():
    ...
    flows = counts["media(SR).m2"].to_numpy()
    ...
    assert reference.attrs["source_2026"]["window_total_plm2"] == pytest.approx(7041.33333371)
    np.testing.assert_allclose(
        reference.set_index("Julian_days").loc[dates.dt.dayofyear, "Progreso_2026"],
        flows.cumsum() / flows.sum(),
    )
    assert reference.loc[reference.Julian_days.lt(10), "Progreso_2026"].isna().all()
    assert reference.loc[reference.Julian_days.lt(10), "N_Campanas_Dia"].eq(8).all()
...
def test_2026_total_is_unavailable_before_last_visit_even_if_future_counts_change(local_files):
    ...
    counts.loc[len(counts) - 1, "media(SR).m2"] = 999999.
    ...
    available = load_local_seasonal_reference(local_files, "2026-09-14")
...
def test_invalid_counts_fail_instead_of_silently_changing_the_pool(local_files, fault):
    ...
    if fault == "duplicate":
        ...
        counts.loc[0, "Fecha"] = "2025-01-10"
    else:
        counts.loc[0 if fault == "no_initial_zero" else 2, "media(SR).m2"] = {
