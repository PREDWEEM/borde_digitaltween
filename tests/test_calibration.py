...
def saved_profile():
    return load_site_profile(DATA / "bordenave_2026.json")


def apply(frame=None, **kwargs):
    arguments = {
        "site": "Bordenave", "as_of": "2027-01-08",
...
def test_invalid_profile_cannot_change_outputs():
    ...
    with pytest.raises(ValueError, match="finitos"):
        apply_site_calibration(example_trajectory(), profile, site="Bordenave", as_of="2027-01-08")
...
def real_data():
    ...
    raw = pd.read_csv(DATA / "bordenave_2026_counts.csv")
...
def test_fit_matches_persisted_profile_and_does_not_mutate_network(real_data):
    ...
    profile, comparison = fit_site_calibration(trajectory, prepared, site="Bordenave")
    saved = saved_profile()
...
def test_invalid_training_data_are_rejected(real_data, fault):
    ...
    with pytest.raises(ValueError):
        fit_site_calibration(trajectory, obs, site="Bordenave")
